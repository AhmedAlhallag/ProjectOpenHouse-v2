...
//============================ submit new OR updated exam ====================S
    $('#exam_form').on('submit', async function(event){
        ...
        if (title && some_date && some_time && dur && question && right_ans && wrong_ans){
            ...
            await sendDataToPage('/OnlineQuizSystem/' + sub_str + '_exam_action_ajax.php',form,axioxParams)
            .then( (data)=>{
    ...
    $(document).on('click','.edit', async(e)=>{
        ...
        await sendDataToPage('/OnlineQuizSystem/edit_exam_action_ajax.php',"",axioxParams)
        .then((data)=>{
