// Create a socket for recieving the objects/messages
...
$(document).ready(() => {
    ...
    if (current_userId != null && current_type == "user") {
        ...
        var recievedObj = ""
        // if we recieved reload; then reload the data table 
        socket.onmessage = function (e) {
            recievedObj = JSON.parse(e.data);
            console.log(recievedObj);
            if (recievedObj.action == "reload") {
                dataTable.ajax.reload();
                console.log(recievedObj.details);
                console.log(recievedObj.action);
                recievedObj = "done";
            }
        }
